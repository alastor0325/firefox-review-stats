...
- **`copilot` (CI default) — no key stored.** Shells out to the [GitHub Copilot CLI](https://docs.github.com/en/copilot/how-tos/copilot-cli/use-copilot-cli-in-actions) (`npm install -g @github/copilot`), which authenticates with the workflow's own `GITHUB_TOKEN` given `permissions: copilot-requests: write` — nothing third-party is stored. Default model **`auto`** (Copilot picks) — pinned ids are gated by the token's Copilot plan and fail hard with `Model … is not available` before any request, so `auto` is the only universally-safe default; override with `REVIEW_STATS_SUMMARY_MODEL` if you have a known-good id. **Not free:** calls consume GitHub AI Credits, billed to the repo owner. The content-hash cache below keeps the weekly spend to genuinely-new feature areas only. Each area is one non-interactive `copilot -p … -s` run. Copilot is an *agent*, not a completion endpoint, so the invocation deliberately declaws it: `--available-tools=` (empty) exposes no tools to the model — which also keeps their schemas out of every billed request — `--disable-builtin-mcps` skips spawning the GitHub MCP server, `--no-custom-instructions` stops a stray `AGENTS.md` rewording the overviews, and the process runs in a throwaway empty directory rather than the checkout. Budget ~20-40s per uncached area; the job's `timeout-minutes` accounts for it.
