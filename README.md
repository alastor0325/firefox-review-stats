...
- **Anthropic Claude API** (optional, run locally) for the Recent Changes per-area overviews. One overview per feature area per window, reasoned over the area's change set, cached by content hash in the git-tracked `.summary_cache/` so CI reuses overviews generated locally without needing the key. See [Recent-change summaries](#recent-change-summaries).
...
The Recent Changes tab's per-area overviews are generated by the Claude API in `analyze_git.py`. They're **optional** — with no key, the tab still renders the patch lists.

**The key is deliberately kept off GitHub**, so overviews are generated **locally** and reused by CI via a committed cache:

- **`.summary_cache/` is tracked in git.** Each overview is cached there keyed by a content hash (feature area + its set of patches). The weekly CI run reads this cache and reuses any overview already generated — **without needing the key in CI**. Only feature-areas never summarized locally show blank until your next local refresh.
- **Refresh locally:** run [`./refresh-overviews.sh`](refresh-overviews.sh) with your key — it pulls, regenerates the reports + overviews, commits the new cache entries, and pushes:
  ```bash
  pip install anthropic            # once
  ANTHROPIC_API_KEY=sk-ant-... ./refresh-overviews.sh
  ```
  Run it whenever you want fresh prose (it also picks up new landings). Cadence is up to you — the data/charts refresh weekly in CI regardless.
- **Model:** defaults to `claude-opus-4-8` (adaptive thinking, so it reasons over the area's changes). Override with `REVIEW_STATS_SUMMARY_MODEL` (e.g. `claude-haiku-4-5` for ~5× lower cost).
- **If you'd rather automate it fully:** set `ANTHROPIC_API_KEY` as a repository secret — the workflow already passes it to `analyze_git.py`, which then fills any gaps the cache doesn't cover.
...
The Recent Changes overviews are generated locally (the key is kept off GitHub) and reused by CI from the committed `.summary_cache/`. CI runs `analyze_git.py` without the key and simply reuses what's cached. See [Recent-change summaries](#recent-change-summaries).
